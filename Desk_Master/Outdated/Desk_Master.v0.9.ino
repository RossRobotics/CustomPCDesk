...
//  6-16-22
//    -Removed icons from GUI as they were making random pins/buttons not work. lol yea
//
...
#pragma pack(push, 1)
uint8_t RemoteXY_CONF[] =   // 986 bytes
  { 255,27,0,6,0,211,3,16,25,4,130,0,30,23,36,34,2,180,130,0,
  ...
  1,4,34,31,32,32,32,32,77,66,32,32,32,32,32,0,31,10,117,2,
  43,13,6,1,4,34,31,32,32,87,97,116,101,114,32,32,0,31,10,117,
  2,50,13,6,1,4,34,31,32,32,87,104,105,116,101,32,32,0,31,10,
  117,30,29,13,6,1,4,34,31,32,73,110,102,105,110,105,116,121,32,0,
  31,129,0,44,19,11,2,0,25,32,87,97,116,101,114,32,70,108,111,119,
  0,1,1,18,30,8,4,1,28,192,226,150,186,0,129,0,18,28,8,2,
  1,26,80,97,116,116,101,114,110,115,0,1,1,18,37,8,4,1,28,136,
  226,150,186,0,1,1,18,44,8,4,1,28,66,226,150,186,0,1,1,18,
  51,8,4,1,26,31,88,0,1,1,46,30,8,4,1,28,163,226,150,186,
  0,129,0,80,1,14,3,0,26,32,80,67,32,82,101,115,101,116,0,129,
  0,82,19,11,2,0,26,80,114,101,115,115,32,52,32,115,101,99,0,129,
  0,4,1,15,3,0,26,32,80,67,32,80,111,119,101,114,0,131,1,20,
  58,19,4,1,14,24,77,97,105,110,0,131,0,42,58,18,4,2,166,24,
  69,102,102,101,99,116,115,0,10,117,30,36,13,6,1,4,34,31,32,32,
  67,97,115,101,32,32,0,31,129,0,37,23,23,3,2,26,67,105,114,99,
  117,105,116,32,86,97,114,105,97,98,108,101,115,0,130,0,66,23,34,34,
  2,180,1,1,46,37,8,4,1,28,227,226,150,186,0,129,0,63,23,29,
  4,1,26,83,112,101,99,105,97,108,32,69,102,102,101,99,116,115,0,129,
  0,5,23,21,3,2,26,76,111,103,111,32,86,97,114,105,97,98,108,101,
  115,0,129,0,10,55,5,2,2,26,67,111,108,111,114,0,4,0,10,25,
  5,29,2,27,25,10,117,30,43,13,6,1,4,34,31,32,67,105,114,99,
  117,105,116,32,0,31,1,1,46,44,8,4,1,28,9,226,150,186,0,4,
  0,33,24,5,29,2,27,25,130,0,57,41,43,16,1,30,1,1,61,50,
  13,5,1,48,31,32,32,32,32,32,85,112,32,32,32,32,32,0,1,1,
  84,50,13,5,1,48,31,32,32,68,111,119,110,32,32,0,129,0,67,41,
  20,4,1,26,32,71,108,97,115,115,32,84,111,112,0,70,17,64,47,6,
  2,1,26,37,0,70,17,87,47,6,2,1,26,37,0,129,0,40,54,10,
  2,2,26,32,65,110,105,109,97,116,105,111,110,0,129,0,33,54,5,2,
  2,26,67,111,108,111,114,0,4,0,44,24,5,29,2,27,25,4,0,55,
  24,5,29,2,27,25,129,0,52,54,6,2,2,26,32,83,119,105,112,101,
  0,1,1,66,30,25,5,1,34,31,32,84,104,117,110,100,101,114,32,83,
  116,111,114,109,32,0 };
...
struct {
  ...
  uint8_t pushSwitch_Circuit; // =1 if state is ON, else =0 
  uint8_t circuitPatternSelect; // =1 if button pressed, else =0 
  int8_t circuit_ColorHue; // =0..100 slider position 
  uint8_t button_Up; // =1 if button pressed, else =0 
  uint8_t button_Down; // =1 if button pressed, else =0 
  int8_t circuit_AnimationSpeed; // =0..100 slider position 
  int8_t circuit_SwipeSpeed; // =0..100 slider position 
  uint8_t storm; // =1 if button pressed, else =0 
  ...
  uint8_t led_Down; // led state 0 .. 1 
...
#define PIN_MBPATTERNSELECT 35 //33,34
#define PIN_WATERPATTERNSELECT 38 //36,37
...
void loop() {
  ...
  digitalWrite(PIN_STORM, (RemoteXY.storm==0)?LOW:HIGH);
   if (RemoteXY.storm!=0) {
    thunderstorm();
   }
  ...
  if (digitalRead(PIN_STOP_AUDIO) == HIGH){
    digitalWrite(52, LOW); //need to test trigger to see how it stops output
  }
  }
