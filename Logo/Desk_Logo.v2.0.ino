...
void fireBlue() {
  ...
  for (int i = 0; i < NUM_COLS; i++) {
    for (int j = 0; j < NUM_ROWS; j++) {
      leds[XY(i, j)] =
        ColorFromPalette(myPal, qsub8(inoise8(i * scalexy, j * scalexy + (int)a, (int)a / 2), abs8(i - (NUM_ROWS - 1)) * 255 / (NUM_ROWS + 2)), 75);
    }
  }
}
...
void fireRed() {
  ...
  for (int i = 0; i < NUM_COLS; i++) {
    for (int j = 0; j < NUM_ROWS; j++) {
      leds[XY(i, j)] =
        ColorFromPalette(myPal2, qsub8(inoise8(i * scalexy, j * scalexy + (int)a, (int)a / 2), abs8(i - (NUM_ROWS - 1)) * 255 / (NUM_ROWS + 2)), 75);
    }
  }
}
