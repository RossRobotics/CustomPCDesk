...
//        +All the other pattern select buttons has been changed to active LOW.
...
void loop() {
  ...
  digitalWrite(PIN_LOGOPATTERNSELECT, (RemoteXY.logoPatternSelect == 0) ? HIGH : LOW);
  analogWrite(LOGO_COLORHUE, RemoteXY.logo_ColorHue * 2.55);
  
  digitalWrite(PIN_MBPATTERNSELECT, (RemoteXY.mbPatternSelect == 0) ? HIGH : LOW);
  digitalWrite(PIN_WATERPATTERNSELECT, (RemoteXY.waterPatternSelect == 0) ? HIGH : LOW);
  digitalWrite(PIN_BUTTONNC, (RemoteXY.buttonNC == 0) ? HIGH : LOW);
  digitalWrite(PIN_INFINITYPATTERNSELECT, (RemoteXY.infinityPatternSelect == 0) ? HIGH : LOW);
  digitalWrite(PIN_PUSHSWITCH_CASELED, (RemoteXY.pushSwitch_CaseLED == 0) ? HIGH : LOW);
  digitalWrite(PIN_CASEPATTERNSELECT, (RemoteXY.casePatternSelect == 0) ? HIGH : LOW);
